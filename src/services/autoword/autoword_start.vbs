' autoword_start.vbs - start the MMA autoword suggester silently.
'
' Let MMA launch it (Settings -> Features -> "Autoword", or the Tools window), or
' double-click it. It runs autoword.pyw under pythonw.exe, so there is NO console
' window: it sits in the background like the pinger and typelog and logs its
' operational lines to MMA's error_log.txt.
'
' A .cmd would flash a console for a moment even with `start`; wscript does not.
'
' Single-instance is enforced inside autoword.pyw (a named mutex, same idea as
...
'
' Settings live in userdata\autoword.ini - written with commented defaults the
' first time this runs. Start with Render=off; see README.md.
...
script = fso.BuildPath(here, "autoword.pyw")

If Not fso.FileExists(script) Then
    MsgBox "Cannot find:" & vbCrLf & script, vbCritical, "MMA autoword"
    WScript.Quit 1
End If
