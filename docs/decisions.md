# MMA — design decisions, and why the tree looks like this

This is the **record of arguments**, not a description of the code. It was the second
half of `ARCHITECTURE.md` while the v2 restructure was being decided; the restructure
shipped in 2.0.0, so it moved here.

Read it when you want to know **why** something is the way it is — why there is one mass
engine instead of three model scripts, why `paths.ahk` exists, what breaks if you move a
file. For **what exists right now**, read [ARCHITECTURE.md](../ARCHITECTURE.md).

Everything below is preserved as it was written, in the future tense it was written in.
Where it says "proposed", "we will" or "delete", that work is **done** — the tree it
argues for is the tree on disk today. Treat tense as a fossil, not an instruction.
...
3. [branching_feature.md](proposals/branching.md) wants a **web** editor that emits mass
...
# Appendix — subsystem rationale

The three sections below describe subsystems that shipped. They are kept here in full
because the *reasoning* is the valuable part and does not belong in an inventory.
[ARCHITECTURE.md](../ARCHITECTURE.md) states what each one does; this says why.
...
> **Seven, since this was written.** `LOGD` / `DEVL` was added later — a breadcrumb
> saying a handler was entered, plus `LOG_Heartbeat` for things on a timer. It is
> written at the normal level, not behind max logging, because a breadcrumb nobody
> can see by default is not a breadcrumb. `src/core/log.ahk`'s own header carried the
> same "six" until a comment audit on 2026-08-27; it lists all seven now.
