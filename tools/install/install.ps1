<#
    ...
    ───────────────────────
      AutoHotkey v2   REQUIRED. Everything core is AHK: masses, follow-ups, alts,
                      ...
                      OCR — srcendor\OCR.ahk drives the OCR engine already built into
...
# install.ps1 lives in tools\install\, so the repo root is two levels up.
# Everything below joins onto $root, so this one line is what moves the
# installer if the file is ever relocated again.
...
$cfg = Join-Path $root 'userdata\mass_gui.cfg'
if (-not (Test-Path $cfg)) {
    Warn 'userdata\mass_gui.cfg not found — MMA will create it with defaults on first run.'
    if (-not $pythonReady) {
...
Say '  --------------------------------------------------------------'
if ($ahk) {
    Say '  MMA is ready. Launch it from the desktop shortcut, or run MMA.ahk.'
    if ($pythonReady) {
