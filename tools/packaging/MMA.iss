...
Name: "{group}\MMA";              Filename: "{code:GetAhkExe}"; Parameters: """{app}\MMA.ahk"""; WorkingDir: "{app}"; IconFilename: "{app}\assets\icon.ico"
Name: "{autodesktop}\MMA";        Filename: "{code:GetAhkExe}"; Parameters: """{app}\MMA.ahk"""; WorkingDir: "{app}"; IconFilename: "{app}\assets\icon.ico"; Tasks: desktopicon

[Run]
Filename: "{code:GetAhkExe}"; Parameters: """{app}\MMA.ahk"""; WorkingDir: "{app}"; \
...
; only these three left the whole program behind (MMA.ahk, src\, all
...
begin
  ...
  // program files. MMA.ahk is the honest signal: the program is not removed
  // until it is gone.
  Waited := 0;
  while (FileExists(ExistingUninstaller) or FileExists(Dir + '\MMA.ahk'))
  ...
  if FileExists(ExistingUninstaller) or FileExists(Dir + '\MMA.ahk') then
...
var
  Cfg: String;
begin
  Cfg := ExpandConstant('{app}\userdata\mass_gui.cfg');
...
begin
  // Two FOLDERS now, where v1 had to name nine files. userdata\ holds every
  // setting, message and log; content\ holds the masses and hotstring library.
  // A folder-level rule cannot go stale when a new settings file is added, which
  // the old file list did every single time.
  Result :=
    (CompareText(Name, 'userdata') = 0) or
    (CompareText(Name, 'content')  = 0);
end;
